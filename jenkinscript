    ...
    stages {
        ...
        stage('Check Apache Status') {
            steps {
                ...
                sh 'ansible 192.168.56.130 -i inventory.yaml -m shell -a "systemctl status httpd"'
                sh 'ansible 192.168.56.132 -i inventory.yaml -m shell -a "systemctl status httpd"'
            }
        }
    }
}
